...
1. call, apply or bind, 函数内的 this 是作为参数传入的对象。
1. obj.method(), 函数内的 this 将绑定到 obj 对象。
1. 如果不符合上述规则，this 的值指向全局对象。浏览器下 this 指向 window 对象，在严格模式 use strict 下，this 指向 undefined.
1. 如果符合上述多个规则，则较高的规则（1 号最高，4 号最低）将决定 this 的值。
1. 如果是箭头函数，this 被设置为它被创建时的上下文。
...
1. `this.__proto__ = Constructor.prototype; this.constructor = Constructor;`
1. 如果 Constructor 内 return 的不是对象、数组或函数，则 return this.
1. 如果 Constructor 内没有 return, 则 return this.
...
比如 forEach, map, filter, reduce, Function.prototype.bind 都是高阶函数。
